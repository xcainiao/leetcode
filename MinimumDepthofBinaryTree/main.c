...
int minDepth1(struct TreeNode* tree) {
    int rightdepth = 0;
    int leftdepth = 0;
    if(!tree)
        return 9999;
    if(!tree->right && !tree->left)
        return 1;
    leftdepth = minDepth1(tree->left);
    rightdepth = minDepth1(tree->right);
    return leftdepth<rightdepth?leftdepth+1:rightdepth+1;
}

int minDepth(struct TreeNode* tree) {
    int result1;
    int result2;
    if(!tree)
        return 0;
    if(!tree->right && !tree->left)
        return 1;
    result1 = minDepth1(tree->left);
    result2 = minDepth1(tree->right);
    return (result1>result2?result2:result1)+1;
}
int main() 
{   
    int depth;
